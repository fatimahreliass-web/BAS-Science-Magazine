...
VALUES (1, 1, 'September', 2026, 'BAS Science Magazine - Issue 1', '???? BAS ??????? - ????? ?????', 'The first student-run science publication of BAS.', '????? ????? ?? ???? BAS ??????? ?? ????? ??????.', NULL, NULL, NULL, 'draft', 1, '2026-09-19 10:33:26')
ON CONFLICT (id) DO NOTHING;
